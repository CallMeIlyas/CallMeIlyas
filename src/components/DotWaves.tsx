...
interface Config {
  dotColor: string; dotRadius: number;
...
const DotWaves: React.FC<DotWavesProps> = ({
  dotColor = 'rgba(52, 53, 53, 0.28)',  // ← naik dari 0.18 → 0.28, lebih kontras tapi masih subtle
  dotRadius = 1.5,
  ...
  const configRef = useRef<Config>({ dotColor, dotRadius, waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, friction, tension, maxCursorMove, xGap, yGap });
  const frameIdRef = useRef<number | null>(null);

  useEffect(() => {
    configRef.current = { dotColor, dotRadius, waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, friction, tension, maxCursorMove, xGap, yGap };
  }, [dotColor, dotRadius, waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, friction, tension, maxCursorMove, xGap, yGap]);
  ...
  useEffect(() => {
    ...
    function updateBounding() {
      if (!container) return;
      // FIX: selalu ambil bounding rect terbaru agar koordinat mouse akurat
      // walau container sudah di-scroll atau di-translate
    ...
    function drawDots() {
      ...
      if (!ctx) return;
      const { dotColor, dotRadius } = configRef.current;
    ...
    function updateMouse(x: number, y: number) {
      // FIX: panggil updateBounding() setiap mouse move
      // agar koordinat tetap akurat meski section di-scroll / di-translate
      updateBounding();
      const mouse = mouseRef.current;
